...
def upsertToDelta(microBatchOutputDf,batchId):
    ...
    try:
        ...
        hash_exclude_col = json.loads(hash_exclude_columns)
        ...
        microBatchOutputDf =microBatchOutputDf.withColumn("is_deleted",  lit(0))
        ...
        if source_optimize[tgt_table_name]['hwm_column'] != '':
            hwm = microBatchOutputDf.select(expr('cast(max({col}) as string)'.format(col=source_optimize[tgt_table_name]['hwm_column'])).alias('hwm')).first()[0]
            logger.info('setting hwm')
            res=set_hwm('bronze',tgt_table_name,hwm,pid)
            logger.info(res)
